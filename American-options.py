...
# Streamlit App
st.subheader("Option Pricing Tool (Binomial Model)")
...
with col_plot:
    ...
    if x_axis_variable == "Spot Price (S0)":
        ...
        if visualization == "Delta":
            ...
            call_values = np.array([price(s, K, T, r, sigma,steps, option_style=option_style, option_type="call") for s in x_values])
            put_values = np.array([price(s, K, T, r, sigma, steps,option_style=option_style, option_type="put") for s in x_values])
        ...
        if visualization == "Delta":
            call_values = np.array([delta(S0, k, T, r, sigma,steps, option_style=option_style, option_type="call") for k in x_values])
            ...
            call_values = np.array([price(S0, k, T, r, sigma,steps, option_style=option_style, option_type="call") for k in x_values])
            put_values = np.array([price(S0, k, T, r, sigma, steps,option_style=option_style, option_type="put") for k in x_values])
        ...
        if visualization == "Delta":
            call_values = np.array([delta(S0, K, T, rate, sigma,steps, option_style=option_style, option_type="call") for rate in x_values])
            put_values = np.array([delta(S0, K, T, rate, sigma,steps, option_style=option_style, option_type="put") for rate in x_values])
        elif visualization == "Gamma":
            call_values = np.array([gamma(S0, K, T, rate, sigma,steps, option_style=option_style, option_type="call") for rate in x_values])
            put_values = np.array([gamma(S0, K, T, rate, sigma, steps,option_style=option_style, option_type="put") for rate in x_values])
        elif visualization == "Theta":
            call_values = np.array([theta(S0, K, T, rate, sigma,steps, option_style=option_style, option_type="call") for rate in x_values])
            put_values = np.array([theta(S0, K, T, rate, sigma,steps, option_style=option_style, option_type="put") for rate in x_values])
        elif visualization == "Price":
            call_values = np.array([price(S0, K, T, rate, sigma,steps, option_style=option_style, option_type="call") for rate in x_values])
            put_values = np.array([price(S0, K, T, rate, sigma,steps, option_style=option_style, option_type="put") for rate in x_values])
        ...
        if visualization == "Delta":
            call_values = np.array([delta(S0, K, time, r, sigma,steps, option_style=option_style, option_type="call") for time in x_values])
            put_values = np.array([delta(S0, K, time, r, sigma,steps, option_style=option_style, option_type="put") for time in x_values])
        elif visualization == "Gamma":
            call_values = np.array([gamma(S0, K, time, r, sigma,steps, option_style=option_style, option_type="call") for time in x_values])
            put_values = np.array([gamma(S0, K, time, r, sigma,steps, option_style=option_style, option_type="put") for time in x_values])
        elif visualization == "Theta":
            call_values = np.array([theta(S0, K, time, r, sigma,steps, option_style=option_style, option_type="call") for time in x_values])
            put_values = np.array([theta(S0, K, time, r, sigma, steps,option_style=option_style, option_type="put") for time in x_values])
        elif visualization == "Price":
        ...
        if visualization == "Delta":
            call_values = np.array([delta(S0, K, T, r, vol, steps,option_style=option_style, option_type="call") for vol in x_values])
            put_values = np.array([delta(S0, K, T, r, vol,steps, option_style=option_style, option_type="put") for vol in x_values])
        elif visualization == "Gamma":
            call_values = np.array([gamma(S0, K, T, r, vol,steps, option_style=option_style, option_type="call") for vol in x_values])
            put_values = np.array([gamma(S0, K, T, r, vol,steps, option_style=option_style, option_type="put") for vol in x_values])
        elif visualization == "Theta":
...
st.markdown("""
1. **Make sure all formulas are correct**
2. **Add rho and vega to the metrics**
3. **Add evaluation using Monte Carlo and Black-Scholes for European options**
